...
function initMFAHajackValidator(options) {
    ...
    const listener = (event) => {
        ...
        (async () => {
            try {
                const uid = options.uidSelector ? (0, utils_1.getFieldValue)(options.uidSelector) : null;
...
function openModal(options, mfaSessionId, getPendingElement, clearPendingElement, onVerifySuccess) {
    var _a, _b, _c, _d;
    const isMobile = (0, utils_1.isMobileDevice)();
    const config = (0, utils_1.getResponsiveConfig)(isMobile);
    ...
    confirmButton.addEventListener('click', async () => {
        ...
        try {
            ...
            if (!isSuccess) {
                // 返回值作为错误信息显示
                message.textContent = result || ((_a = options.errorText) !== null && _a !== void 0 ? _a : '验证码错误，请重试');
                input.value = '';
                input.focus();
                return;
            }
